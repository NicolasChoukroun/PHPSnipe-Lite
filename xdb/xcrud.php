...
error_reporting(E_ERROR); // error reporting (debug)
...
class Xcrud
{
    ...
    protected $upload_folder_def = '../images';
    ...
    {
        self::init_prepare();
	    if (!$name) $name = sha1(rand() . microtime());
	    //if (!$name) $name = "alskjdlaksjldkjasllkjl5646546#$#$";
    ...
    public static function get_requested_instance()
    {
        if (isset($_POST['xcrud']['instance']) && isset($_POST['xcrud']['key']) && isset($_POST['xcrud']['task']))
        {
	        ...
	        $key="alskjdlaksjldkjasllkjl5646546#$#$";
    ...
    protected static function init_prepare($method = false)
    {
        switch ($method)
        {
            ...
            case 'get':
                ...
                break;
	        case 'session':
		        $sess_name = (Xcrud_config::$dynamic_session && isset($_SESSION['xcrud']['sess_name']) && $_SESSION['xcrud']['sess_name']) ? $_SESSION['xcrud']['sess_name'] :
			        Xcrud_config::$sess_name;
		        break;
            default:
    ...
    protected function _render_file()
    {
        ...
        else
        {
            ...
            @readfile($path) or die("File not found at ".$path);
        }
    ...
    protected function _regenerate_key()
    {
        switch ($this->task)
        {
            ...
            default:
	            ...
	            //$this->key="alskjdlaksjldkjasllkjl5646546#$#$";
                break;
        }
    }
    ...
    protected function create_bool($name, $value = '', $tag = array())
    {
        ...
        return $this->open_tag('div', $this->theme_config('checkbox_container')) . $this->open_tag('label') . $this->single_tag($tag, $this->theme_config('bool_field'),
		        $this->field_attr[$name], true) . $this->close_tag('label') . $this->close_tag('div');
    ...
    protected function create_image($name, $value = '', $tag = array(), $is_upload = false)
    {
        ...
        if ($value)
        {
			...
			//$out.="test=".$this->upload_config[$name][1];
    ...
    { // loads theme configuration from ini file
        if (is_file(XCRUD_PATH . '/' . Xcrud_config::$themes_path . '/' . $this->theme . '/xcrud.ini'))
            ...
            self::error('xcrud.ini does not exist in your theme folder'." - ".XCRUD_PATH . '/' . Xcrud_config::$themes_path . '/' . $this->theme . '/xcrud.ini');
    ...
    public static function load_css()
    {
        ...
        {
            Xcrud_config::$scripts_url = self::check_url(Xcrud_config::$scripts_url, true);
            Xcrud_config::$user->is_editor_url = self::check_url(Xcrud_config::$user->is_editor_url);
            Xcrud_config::$user->is_editor_init_url = self::check_url(Xcrud_config::$user->is_editor_init_url);
        }
        ...
        if (Xcrud_config::$load_jcrop)
            ...
            $out .= '<link href="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri . '/timepicker/jquery-ui-timepicker-addon.css" rel="stylesheet" type="text/css" />';
           // $out .= '<link href="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$themes_uri . '/' . Xcrud_config::$theme . '/xcrud.css" rel="stylesheet" type="text/css" />';
        return $out;
    }
    public static function load_js()
    {
        if (self::$js_loaded==true) return "";
        $out = '';
        if (self::$instance)
        {
    ...
    {
        if (isset($this->upload_folder[$field])) return $this->upload_folder[$field];
